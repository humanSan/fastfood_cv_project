"""
Evaluation script for the FastSAM variant of Semantic-FastSAM.

Identical metrics to evaluate.py but uses the FastSAMPipeline and saves
all outputs to `evaluation_metrics_fastsam/` for side-by-side comparison.

Usage:
    python eval_fastsam.py --data_dir data/FoodSeg103 --split test
"""
...
def plot_per_class_metric(metric_values, metric_name, out_path, top_n=30):
    ...
    ax.set_title(f"Per-Class {metric_name} (Top {len(class_names)} — FastSAM)",
...
def plot_confusion_matrix(confusion, out_path, top_n=25):
    ...
    ax.set_title(f"Normalised Confusion Matrix — FastSAM (Top {top_n})",
...
def plot_latency_histogram(latencies, out_path):
    ...
    ax.set_title("Inference Latency Distribution — FastSAM", fontsize=14, fontweight="bold")
    ax.legend()
...
def save_sample_predictions(images, gt_maps, pred_maps, out_dir, n=6):
    ...
    for i in range(n):
        ...
        axes[i, 2].set_title("Prediction (FastSAM)", fontsize=12, fontweight="bold")
        axes[i, 2].axis("off")

    fig.suptitle("FastSAM Pipeline — Sample Predictions", fontsize=16, fontweight="bold", y=1.02)
    fig.tight_layout()
...
def evaluate(args):
    ...
    for i in tqdm(range(num_to_eval), desc="Evaluating (FastSAM)"):
        sample = dataset[i]
        image = sample["image"]
        gt_mask = sample["mask"]
    ...
    print("\n" + "=" * 60)
    print(f"  FastSAM Evaluation Results  ({len(latencies)} valid images)")
    print("=" * 60)
    ...
    with open(report_path, "w") as f:
        f.write("Semantic-FastSAM (Ultralytics) — Evaluation Report\n")
        f.write("=" * 55 + "\n\n")
    ...
    print(f"\n[Saved] All FastSAM evaluation artifacts saved to: {args.out_dir}/")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate Semantic-FastSAM (Ultralytics) on FoodSeg103"
    )
    ...
    parser.add_argument("--out_dir", default="evaluation_metrics_fastsam",
                        help="Directory to save results")
    parser.add_argument("--fastsam_ckpt", default="FastSAM-x.pt", help="Ultralytics FastSAM weights")
