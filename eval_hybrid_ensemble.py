"""
Evaluation script for the Hybrid Ensemble Pipeline.
Provides comprehensive metrics, profiling, and visualisations.
"""
...
def plot_per_class_metric(metric_values, metric_name, out_path, top_n=30):
    ...
    ax.set_title(f"Per-Class {metric_name} (Top {len(class_names)} — Hybrid)",
...
def plot_confusion_matrix(confusion, out_path, top_n=25):
    ...
    ax.set_title(f"Normalised Confusion Matrix — Hybrid (Top {top_n})",
...
def save_sample_predictions(images, gt_maps, pred_maps, out_dir, n=6):
    ...
    for i in range(n):
        ...
        axes[i, 2].set_title("Prediction (Hybrid)", fontsize=12, fontweight="bold")
        axes[i, 2].axis("off")

    fig.suptitle("Hybrid Ensemble Pipeline — Sample Predictions", fontsize=16, fontweight="bold", y=1.02)
    fig.tight_layout()
...
def evaluate(args):
    ...
    print(f"Evaluating {num_to_eval} samples using Hybrid Ensemble (SegFormer + YOLO26-{args.model})")
    if not args.no_masks:
        print("Using MobileSAM for mask-based fusion.")
    ...
    # Profiling Report
    print("\n" + "="*40)
    print("  PIPELINE PROFILING SUMMARY (Avg)")
    print("="*40)
    for k, v in stage_times.items():
        avg = np.mean(v)
        pct = (avg / np.mean(stage_times["total"])) * 100
        print(f"  {k:12s}: {avg:6.3f}s ({pct:5.1f}%)")
    print("="*40)

    print("\n" + "=" * 60)
    print(f"  Hybrid Ensemble Evaluation Results  ({len(stage_times['total'])} images)")
    print("=" * 60)
    print(f"  mIoU:              {miou * 100:.2f}%")
    print(f"  mAcc (Class Acc):  {macc * 100:.2f}%")
    print(f"  Mean Dice:         {mdice * 100:.2f}%")
    print(f"  Pixel Accuracy:    {pixel_acc * 100:.2f}%")
    avg_latency = np.mean(stage_times["total"])
    print(f"  Avg Latency:       {avg_latency:.3f}s  (FPS: {1.0 / avg_latency:.1f})")
    print("=" * 60)

    # ── Generate Plots & Reports ───────────────────────────────────────
    print("\nGenerating presentation charts...")
    plot_per_class_metric(per_class_iou, "IoU", os.path.join(args.out_dir, "per_class_iou.png"))
    plot_per_class_metric(per_class_dice, "Dice Score", os.path.join(args.out_dir, "per_class_dice.png"))
    plot_per_class_metric(per_class_acc, "Class Accuracy", os.path.join(args.out_dir, "per_class_acc.png"))
    plot_confusion_matrix(total_confusion, os.path.join(args.out_dir, "confusion_matrix.png"))
    save_sample_predictions(sample_images, sample_gt, sample_pred, args.out_dir, n=args.save_n_visuals)

    # ── Text Report ────────────────────────────────────────────────────
    report_path = os.path.join(args.out_dir, "evaluation_summary.txt")
    with open(report_path, "w") as f:
        f.write("Semantic-Hybrid-Ensemble — Evaluation Report\n")
        f.write("=" * 55 + "\n\n")
        f.write(f"Dataset Split:    FoodSeg103 ({args.split})\n")
        f.write(f"Images Evaluated: {len(stage_times['total'])}\n")
        f.write(f"Mask Mode:        {'MobileSAM' if not args.no_masks else 'None (Pixel-Fusion)'}\n")
        f.write(f"YOLO Verification: {'OFF' if args.no_yolo else 'ON'}\n")
        if not args.no_masks:
            f.write(f"SAM Prompting:    {'Superpixel' if args.sp_prompt else 'Grid'}\n")
        ...
        f.write(f"Avg FPS:          {1.0 / avg_latency:.1f}\n\n")

        f.write("Stage Profiling (Avg):\n")
        for k, v in stage_times.items():
            f.write(f"  {k:12s}: {np.mean(v):.3f}s\n")
        f.write("\n")
    ...
    results_json = {
        ...
        "profiling": {k: float(np.mean(v)) for k, v in stage_times.items()},
        "per_class_metrics": {
    ...
    print(f"\n[Saved] All hybrid evaluation artifacts saved to: {args.out_dir}/")


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", default="data/FoodSeg103")
    ...
    parser.add_argument("--sf_bias", type=float, default=0.8, help="SegFormer confidence bias")
    parser.add_argument("--max_samples", type=int, default=None, help="Maximum samples to evaluate (default: all)")
    parser.add_argument("--save_n_visuals", type=int, default=6, help="Number of sample predictions to save")
    parser.add_argument("--out_dir", default="eval_output_hybrid")
