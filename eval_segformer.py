"""
Evaluation script for the standalone SegFormer model.
Generates metrics and visualisations.
"""
...
def plot_per_class_metric(metric_values, metric_name, out_path, top_n=30):
    ...
    ax.set_title(f"Per-Class {metric_name} (Top {len(class_names)} — SegFormer)",
...
def plot_confusion_matrix(confusion, out_path, top_n=25):
    ...
    ax.set_title(f"Normalised Confusion Matrix — SegFormer (Top {top_n})",
...
def save_sample_predictions(images, gt_maps, pred_maps, out_dir, n=6):
    ...
    for i in range(n):
        ...
        axes[i, 2].set_title("Prediction (SegFormer)", fontsize=12, fontweight="bold")
        axes[i, 2].axis("off")

    fig.suptitle("SegFormer Standalone — Sample Predictions", fontsize=16, fontweight="bold", y=1.02)
    fig.tight_layout()
...
def evaluate(args):
    ...
    print(f"Evaluating {num_to_eval} samples using Standalone SegFormer")
    ...
    for i in tqdm(range(num_to_eval), desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            pred_map = prob_map.argmax(dim=-1).cpu().numpy().astype(np.uint8)
    ...
    print("\n" + "=" * 60)
    print(f"  SegFormer Evaluation Results  ({len(latencies)} images)")
    print("=" * 60)
    ...
    print(f"  Avg Latency:       {avg_latency:.4f}s  (FPS: {1.0 / avg_latency:.2f})")
    print("=" * 60)
    ...
    with open(report_path, "w") as f:
        f.write("SegFormer Standalone — Evaluation Report\n")
        f.write("=" * 55 + "\n\n")
        ...
        f.write(f"Avg FPS:          {1.0 / avg_latency:.2f}\n\n")
    ...
    print(f"\n[Saved] All SegFormer evaluation artifacts saved to: {args.out_dir}/")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", default="weights/segformer_foodseg103_best")
    ...
    parser.add_argument("--out_dir", default="eval_output_segformer")
