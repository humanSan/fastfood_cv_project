"""
Evaluation script for the Semantic-EfficientSAM3 pipeline on FoodSeg103.

Computes comprehensive metrics suitable for presentation:
  - mIoU (mean Intersection over Union)
  - mAcc (mean Class Accuracy)
  - Pixel Accuracy
  - Mean Dice Coefficient
  - Per-class metrics and Normalised Confusion Matrix
  - Inference Latency and FPS

All charts, plots, and reports are saved to the `evaluation_metrics/` directory.

Usage:
    python evaluate.py --data_dir data/FoodSeg103 --split test
"""
...
def compute_metrics_batch(pred: np.ndarray, target: np.ndarray, num_classes: int):
    ...
    # Confusion matrix elements for this image
...
def plot_per_class_metric(metric_values, metric_name, out_path, top_n=30, color="#4CAF50"):
    """Horizontal bar chart for top N classes of a specific metric (IoU, Dice, etc.)."""
    ...
    ax.set_title(f"Per-Class {metric_name} (Top {len(class_names)} Classes)", fontsize=15, fontweight="bold")
    ax.set_xlim(0, 100)
...
def plot_latency_histogram(latencies, out_path):
    ...
    ax.axvline(np.mean(latencies), color="red", linestyle="dashed", linewidth=2, label=f"Mean: {np.mean(latencies):.3f}s")
...
def save_sample_predictions(images, gt_maps, pred_maps, out_dir, n=6):
    ...
    for i in range(n):
        ...
        gt_colour = colourise_segmentation(gt_maps[i])
        pred_colour = colourise_segmentation(pred_maps[i])
    ...
    fig.suptitle("Pipeline Sample Predictions", fontsize=16, fontweight="bold", y=1.02)
    fig.tight_layout()
...
def evaluate(args):
    ...
    for i in tqdm(range(num_to_eval), desc="Evaluating"):
        ...
        # Run pipeline
        t0 = time.time()
        pred_map, _, _ = pipe.run(image, verbose=False)
    ...
    print("\n" + "=" * 60)
    print(f"  Evaluation Results  ({len(latencies)} valid images)")
    print("=" * 60)
    ...
    plot_per_class_metric(per_class_iou, "IoU", os.path.join(args.out_dir, "per_class_iou.png"), color="#4CAF50")
    plot_per_class_metric(per_class_dice, "Dice Score", os.path.join(args.out_dir, "per_class_dice.png"), color="#2196F3")
    plot_per_class_metric(per_class_acc, "Class Accuracy", os.path.join(args.out_dir, "per_class_acc.png"), color="#9C27B0")
    ...
    with open(report_path, "w") as f:
        f.write("Semantic-EfficientSAM3 — Evaluation Report\n")
        f.write("=" * 55 + "\n\n")
        ...
        for c in range(NUM_CLASSES):
            if not np.isnan(per_class_iou[c]):
                f.write(f"{FOODSEG103_CLASSES[c]:<25s}  {per_class_iou[c]*100:8.2f}  {per_class_dice[c]*100:8.2f}  {per_class_acc[c]*100:8.2f}\n")
    ...
    print(f"\n[Saved] All evaluation artifacts saved to: {args.out_dir}/")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate Semantic-EfficientSAM3 on FoodSeg103")
    ...
    parser.add_argument("--out_dir", default="evaluation_metrics", help="Directory to save results")
